...
class AgbhPeakQcService:
    validation_config: Mapping | None

    def theoretical_q_nm(self) -> np.ndarray:
        cfg = self.validation_config if isinstance(self.validation_config, Mapping) else {}
        return agbh_theoretical_q_nm(str(cfg.get("calibrant") or "AgBh"))

    def evaluate_series(self, series: Iterable[Mapping]) -> list[str]:
        return evaluate_agbh_peak_qc_for_series(
            series,
            validation_config=self.validation_config,
        )

    def evaluate_h5(self, container_path: Path | str, *, schema) -> list[str]:
