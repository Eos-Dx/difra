...
_CONTAINER_INFORMATION_EDIT_PASSWORD = "Ulster2026_REDACTED"
...
class SessionMixin(SessionWorkspaceMixin, SessionFlowMixin):
    ...
    def add_session_menu_actions(self):
        ...
        edit_container_info_action = QAction("Edit Container Information...", self)
        edit_container_info_action.triggered.connect(
            self.on_edit_session_container_information
        )
        edit_container_info_action.setStatusTip(
            "Correct active session Specimen ID, Project, Study/Group, Matador IDs, or Operator"
        )
        file_menu.addAction(edit_container_info_action)
