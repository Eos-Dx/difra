...
class H5LoadingRuntimeRowsMixin:
    ...
    def _json_safe_runtime_value(value):
        ...
        if isinstance(value, dict):
            return {
                str(key): H5LoadingRuntimeRowsMixin._json_safe_runtime_value(item)
                for key, item in value.items()
            }
        if isinstance(value, (list, tuple)):
            return [
                H5LoadingRuntimeRowsMixin._json_safe_runtime_value(item)
                for item in value
            ]
        return value
    @staticmethod
    def _normalize_runtime_row_for_signature(entry):
        ...
        normalized_metadata = {
            "integration_time_ms": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                metadata.get("integration_time_ms")
            ),
            "n_frames": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                metadata.get("n_frames")
            ),
            "thickness": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                metadata.get("thickness")
            ),
        }
        return {
            "alias": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                str(entry.get("alias") or "")
            ),
            "technical_type": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                str(entry.get("technical_type") or "")
            ),
            "is_primary": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                bool(entry.get("is_primary"))
            ),
            "source_ref": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                str(entry.get("source_ref") or "")
            ),
            "source_path": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
                str(entry.get("source_path") or "")
            ),
            "row_id": H5LoadingRuntimeRowsMixin._json_safe_runtime_value(
