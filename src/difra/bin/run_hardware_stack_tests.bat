...
set GLOBAL_CONFIG=%REPO_ROOT%\src\difra\resources\config\global.json
...
%CONDA_CMD% run -n %GUI_ENV% python "%REPO_ROOT%\src\difra\scripts\ensure_runtime_dependencies.py" --require container --require protocol --require xrdanalysis
...
%CONDA_CMD% run -n %GUI_ENV% python -m pytest -q -s ^
  "%REPO_ROOT%\tests\upstream_snapshot\test_detector_integration_timing_e2e.py" ^
  "%REPO_ROOT%\tests\upstream_snapshot\manual_hardware_real_legacy_e2e.py"
