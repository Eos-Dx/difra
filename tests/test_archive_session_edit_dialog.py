...
def test_archive_edit_dialog_blocks_without_runtime_matador_token(qapp, tmp_path, monkeypatch):
    ...
    try:
        ...
        assert dialog.matador_progress_bar.isVisible() is False
    finally:
        dialog.deleteLater()
...
def test_archive_edit_dialog_shows_loading_status_while_refresh_runs(qapp, tmp_path, monkeypatch):
    ...
    def _refresh_stub(**kwargs):
        observed["message"] = dialog.matador_status_label.text()
        observed["progress_hidden"] = dialog.matador_progress_bar.isHidden()
        return {
            "savedAt": "2026-04-23T16:00:00Z",
            "studies": [
                {
    ...
    try:
        qapp.processEvents()
        assert "connecting to matador" in observed["message"].lower()
        assert observed["progress_hidden"] is False
        assert dialog.matador_progress_bar.isVisible() is False
    finally:
        dialog.deleteLater()
